...
= 0.6.0 =
* **WordPress.org compliance.** Defers `load_plugin_textdomain` to the `init` hook (silences WP 6.7+ `_doing_it_wrong` notice), wraps every `$_POST`/`$_GET` access in `wp_unslash()` + `sanitize_*`, validates the admin tab against a whitelist, and moves the inline admin JS into a properly enqueued asset with `wp_localize_script`. The settings page now passes WordPress Plugin Check with zero errors.
